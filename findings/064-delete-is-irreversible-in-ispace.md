# Finding 064: DELETE Is Irreversible in I-Space

**Date:** 2026-02-11
**Status:** Confirmed by code trace + test
**Category:** Specification Semantics, Identity Model
**Sources:** `backend/edit.c:31-76`, `backend/insertnd.c:199-217`, `backend/do1.c:45-65`, Bug 019
**Related:** Finding 030 (INSERT allocates fresh I-addresses), Finding 057 (Spanfilade write-only), Finding 058 (Tree state after delete-all)
...
2. **Spanfilade entries**: The reverse index still records that this document
   once contained I-addresses 5.2–5.3 (Finding 057). No `deletespanf` exists.
...
INSERT always allocates **fresh** I-addresses from the granfilade (Finding 030).
...
From Bug 019: after DELETE removes all content, `findleftson(father)` returns
...
- `insertnd.c:199-217` — `firstinsertionnd`: Bug 019 fix reveals empty-after-delete state
- Finding 030 — INSERT allocates fresh I-addresses
- Finding 057 — Spanfilade is write-only, no cleanup on DELETE
- Finding 058 — Tree state after delete-all (levelpull disabled)
- Bug 019 — INSERT/VCOPY crash after delete-all (triggered this investigation)
