# Finding 054: INSERT Does Not Shift Link Subspace -- The Second Cut Blade

**Date:** 2026-02-07
**Category:** POOM operations / Subspace isolation
**Agent:** Gregory
**Related:** Finding 038, Finding 052, Finding 053
...
### Finding 038 Is Correct

Finding 038 (POOM Subspace Independence) correctly identifies that "operations in the text subspace (INSERT, DELETE) do not renumber or shift link V-positions." This finding provides the detailed code-level mechanism that explains WHY.

### Finding 052 Needs Clarification

Finding 052 (CREATELINK Shifts POOM Entries) correctly identifies that `makegappm` is called during CREATELINK. But the shift from a link creation at 2.x would only affect crums in the 2.x range (other links), not crums in 1.x (text). The second cut blade for an insertion at 2.x would be 3.1, so text crums are safe.

## Related Findings

- **Finding 038**: POOM Subspace Independence (behavioral observation, confirmed here)
- **Finding 052**: CREATELINK Shifts POOM Entries (mechanism analysis, clarified here)
- **Finding 053**: DELETE Shifts V-Positions (analogous mechanism for deletion -- uses `deletecutsectionnd` which likely has similar two-blade structure)
- **Finding 031**: Tumbler Arithmetic (explains the digit-level operations)
