# Finding 0068: VERSION Allocates Addresses as Children of the Source Document
...
From Finding 0021, we know `depth = (supertype == subtype) ? 1 : 2`, so for `DOCUMENT, DOCUMENT`, **depth = 1**.
...
Like element (ATOM) allocation documented in Finding 0061, VERSION uses **stateless query-and-increment**:
...
Like Finding 0061 for elements, **deleting a version does not free its address**. The granfilade tree retains all allocated addresses permanently, so:
...
- **Finding 0021: Address Allocation and Account Boundaries** - Documents how `findisatoinsertnonmolecule` works for DOCUMENT allocation
- **Finding 0061: I-Address Allocation is Monotonic** - Explains the stateless query-and-increment mechanism used for elements
- **Finding 0007: Version Semantics** - Documents version behavior at the operational level (this finding explains the underlying allocation mechanism)
- **Finding 0043: CREATENEWVERSION Copies Text Only** - Documents what content is copied (this finding explains where the new document is allocated)
