...
**Note:** Deep version chains (3+ versions with content added at each level) cause the backend to crash when using `compare_versions` or `find_documents`. See Bug 012.
...
These findings confirm the sporgl architecture documented in Finding 013:
...
- **Finding 002:** Transclusion content identity immutable
- **Finding 012:** Dual enfilade architecture (granf + spanf)
- **Finding 013:** Sporgl provenance tracking

## Related Bugs

- **Bug 012:** Deep version chain crash - backend crashes on 3+ version chains with compare_versions or find_documents
