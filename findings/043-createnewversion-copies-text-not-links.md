# Finding 043: CREATENEWVERSION Copies Text Subspace Only, Not Links
...
Despite the version not having link subspace entries in its POOM, `find_links` still works on the version (as shown in Finding 007 test `version_with_links`). This is because:
...
Document POOM (V-space):
  ...
  2.x     - Link subspace (alternative storage? - see Finding 009)
...
- **Finding 007: Version Semantics** - Links discovered via find_links work on versions through content identity
- **Finding 009: Document Address Space Structure** - POOMs have distinct subspaces for text (1.x) and links (0.x or 2.x)
- **Finding 038: POOM Subspace Independence** - Text and link subspaces are independent trees
...
This explains the apparent contradiction in Finding 007: links "transfer" to versions in the sense that `find_links` works, but they do not transfer at the POOM structural level. The distinction is between **POOM copying** (structural) and **link discovery** (content-identity based).
