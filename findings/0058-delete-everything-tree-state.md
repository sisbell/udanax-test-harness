# Finding 0058: DELETE All Content Tree State — Disabled levelpull Prevents Collapse
...
**Related:** EWD-006 (Enfilade Structure), Finding 0053 (V-Position Shifts)
...
2. **Loop through sons**: Each bottom node in the deletion range:
   ...
   - **Case 2**: Node is after deletion range → shift V-position (Finding 0053)
...
**Related Findings:**
- Finding 0053: V-position shifts during DELETE (tumblersub mechanics)
- Finding 0057: Spanfilade entries not cleaned up on DELETE
...
- **2026-02-11**: Finding 0064 confirms that the empty-after-delete state causes
  INSERT/VCOPY to crash (Bug 0019). The crash was in `firstinsertionnd()` which
  assumed a bottom crum always exists. Fixed by creating a new bottom crum when
  `findleftson()` returns NULL. See Finding 0064 for full analysis including the
