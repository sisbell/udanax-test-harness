# Finding 063: CREATELINK Breaks I-Address Contiguity for Subsequent Text INSERTs
...
The mechanism follows from Finding 052 and Finding 061:

1. **I-address allocation is monotonic** (Finding 061): Each INSERT calls `findpreviousisagr` to find the highest existing I-address, then increments.

2. **CREATELINK allocates in the granfilade** (Finding 052): CREATELINK calls `createorglingranf` which creates a link orgl in the granfilade. This orgl gets an I-address via `findisatoinsertnonmolecule` (for non-ATOM/non-text entities).
...
- **Finding 052**: CREATELINK uses insertpm, shifts POOM entries (the mechanism)
- **Finding 061**: I-address allocation is monotonic, DELETE doesn't affect it (the contrast)
- **Finding 033**: I-space consolidates adjacent addresses (what happens when no gap exists)
- **Finding 062**: Interior typing coalescing via ONMYRIGHTBORDER (what coalescing looks like when it works)
- **Finding 038**: POOM subspace independence (link subspace vs text subspace)
