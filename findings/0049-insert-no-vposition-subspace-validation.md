# Finding 0049: INSERT Does Not Validate V-Position Subspace
...
The vspanset shows TWO spans after the insert. The notation "start": "0" appears to be a normalized representation that encompasses the 2.x range (see Finding 0038).
...
This aligns with Finding 0011 (Convention Over Enforcement Design) - the system relies on front end cooperation for many semantic invariants.
...
From Finding 0033 (EWD-033), CREATELINK uses `NOBERTREQUIRED` when creating link orgls:
...
- **Finding 0011**: Convention Over Enforcement Design - system relies on front end for many invariants
- **Finding 0033**: ENF0 predicate closes convention gap for MODIFICATION but not PLACEMENT
- **Finding 0038**: POOM Subspace Independence - describes 1.x (text) and 2.x (link) subspaces
- **Finding 0042**: BED Event Loop Atomicity - INSERT is atomic at operation level
