# Finding 0065: Link I-Address Allocation is Per-Document, Not Global
...
This is consistent with text I-address allocation (Finding 0061), which also uses query-and-increment within a bounded region. For text:
...
The `granf` is a **single global enfilade** (Finding 0012), but allocation queries are **bounded by document**. This is the same pattern as document allocation under accounts (Finding 0021):
...
- **Finding 0061**: I-address allocation is monotonic (for text, same mechanism applies to links)
- **Finding 0063**: CREATELINK breaks I-address contiguity (for text inserts after link creation)
- **Finding 0021**: Address allocation and account boundaries (hierarchical allocation with bounded queries)
- **Finding 0012**: Dual enfilade architecture (global granf with per-document operations)
