# Finding 061: I-Address Allocation is Monotonic and Unaffected by DELETE
...
This explains **Finding 033** (I-space consolidates adjacent addresses): sequential inserts produce sequential I-addresses, which naturally consolidate into single I-spans.
...
- **Finding 021**: Address allocation mechanism (covers non-ATOM allocation via `findisatoinsertnonmolecule`)
- **Finding 033**: I-space consolidates adjacent addresses (explains why sequential inserts produce single I-spans)
- **Finding 030**: INSERT updates V-space while preserving I-space identity
- **Finding 057**: Spanfilade has no cleanup on DELETE (confirms DELETE is spanfilade-only)
