# Bug 0002: BERTMODEONLY doesn't handle openState==0
...
The `BERTMODEONLY` case in `doopen()` doesn't properly handle `openState==0` (document not yet in bert table but can be opened). After fixing Bug 0001, this case is reached but not handled.
